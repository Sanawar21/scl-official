...
1. **Team identity** — `global_teams` schema + backfill; `create_team` (no purse) / `delete_team` (no clawback); "My team" card on the account page (create team, edit logo/about); `team_profile` reads global data. Tests.
2. **S2 economy rules** — purse removed from ruleset UI + defaults; idempotent **10k funding** action + script; admin grants kept. Tests.
3. **Universal credit + auto mode** — 250 to every player wallet per match; `auto_vault` flag + deposit routing + toggle. Tests.
